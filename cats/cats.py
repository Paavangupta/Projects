...
def pick(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    valid_paras=[i for i in paragraphs if select(i)]
    if k>=len(valid_paras):
        return ''
    else:
        return valid_paras[k]
    # END PROBLEM 1
...
def accuracy(typed, source):
    ...
    if len(typed_words)==0 and len(source_words)==0:
        ...
        if len(typed_words)>len(source_words):
            index_range= len(source_words)
        else:
            index_range=len(typed_words)
        for i in range(index_range):
            if typed_words[i]==source_words[i]:
                correct_counter+=1
        return correct_counter*100/len(typed_words)
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list:
        ...
        hof_diff=lambda x: abs(diff_function(typed_word,x,limit))
        closest_word=min(word_list,key=hof_diff)
        if hof_diff(closest_word)>limit:
            return typed_word
        else:
            return closest_word
    # END PROBLEM 5
...
def furry_fixes(typed, source, limit):
    ...
    if abs(len(typed)-len(source))>limit:
        ...
        if typed[0]==source[0]:
            a=0
        return a+furry_fixes(typed[1:],source[1:],limit-a)
...
def report_progress(typed, source, user_id, upload):
    ...
    for i in range(len(typed)):
        if typed[i]==source[i]:
            ...
            break
    progress= correct_counter/len(source)
...
def time_per_word(words, timestamps_per_player):
    ...
    # BEGIN PROBLEM 9
    hof= lambda x: [x[i]-x[i-1] for i in range(1,len(x))]
    times = [hof(i) for i in tpp]  # You may remove this line
    # END PROBLEM 9
...
def fastest_words(words_and_times):
    ...
    for k in word_indices:
        hof=lambda x: get_time(times,x,k)
        player_fastest=min(player_indices,key=hof)
        ret_list[player_fastest]=ret_list[player_fastest]+ [words[k]]
    return ret_list
    # END PROBLEM 10
